...
def createDict(N=3,h=True,mod=10000):   
    t1 = time.time()
    l=[]
...
def nGrams(token,n,d,h=True,mod=10000):
    l = len(token)
...
def abbrev(token,d,N,h=True):
    ...
    if not abbrev == '':
        ...
        for i in range(2,N):
            if h:
                nGrams(abbrev,i,d,h=True)
            else:
                nGrams(abbrev,i,d)
        
        
def toVector(token,empty,N=3,h=False):
    t1 = time.time()
    d=empty.copy()
